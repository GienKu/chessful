...
/**
 * Schema for validating a friend's invitation response.
 *
 * This schema ensures that the response contains:
 * - `id`: A valid ObjectId.
 * - `isAccepted`: A boolean indicating whether the invitation was accepted.
 */
export const FriendInvitationResponseSchema = v.object({
  id: ObjectIdSchema,
...
export const GetPlayersQuerySchema = v.object({
  username: v.pipe(v.string()),
});

export type ObjectIdData = v.InferOutput<typeof ObjectIdSchema>;
